...
export async function writeLesson(
  ...
  addUsage(usage, response);
  const parsed = parseJsonFromMessage<LessonResponse>(response);

  // Validate citations: every [#sou_id] in the body must resolve to one
  // of the sources we passed in. Strip the ones that don't, and note it
  // in the lesson (we don't silently keep ghosts).
  const allowedIds = new Set(candidateSources.map((s) => s.id));
  const cleanCitations = parsed.citations.filter((c) =>
    allowedIds.has(c.sourceId),
  );

  // Also drop inline citations the body has for non-allowed ids.
  let cleanBody = parsed.body;
  cleanBody = cleanBody.replace(/\[#([^\]]+)\]/g, (full, id: string) => {
    return allowedIds.has(id) ? full : "";
  });
  ...
  return {
    ...
    exercises: [], // filled in by generateExercises
  ...
  const lessons: Lesson[] = [
    {
      ...
      body:
        ...
        `in the way [#${dataset[0]?.id ?? "sou_001"}]. In this lesson you'll ` +
        `map the parts of your week most exposed to AI tooling.\n\n` +
        ...
        `ethics conversation that follows any of those [#${dataset[1]?.id ?? "sou_002"}].\n`,
      citations: [
        {
          sourceId: dataset[0]?.id ?? "sou_001",
          note: "Surveys of where AI shows up in UX practice in 2024–25.",
        },
        {
          sourceId: dataset[1]?.id ?? "sou_002",
